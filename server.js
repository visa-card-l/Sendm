...
// ==================== PAYSTACK CONFIG ====================
const MONTHLY_PRICE = 500000; // â‚¦5,000 in kobo (Paystack uses smallest currency unit)
...
async function executeScheduledBroadcast(broadcastId) {
  ...
  if (result.error) {
    ...
    const statusEmoji = result.failed === 0 ? 'âœ”' : 'âš ï¸';
...
// No automatic redirect after payment â€“ user returns manually
...
app.post('/admin-limits', (req, res) => {
  ...
  if (isNaN(newDaily) || isNaN(newPages) || isNaN(newForms) || newDaily < 1 || newPages < 1 || newForms < 1) {
    return res.send(`
      ...
      <html>
        <head><style>body{background:#121212;color:#f44336;display:flex;justify-content:center;align-items:center;height:100vh;margin:0;font-family:sans-serif;text-align:center;}</style></head>
        <body><h1>Invalid Values<br>All limits must be â‰¥ 1</h1></body>
      </html>
    `);
  }
...
app.listen(PORT, () => {
  console.log('\nSENDEM SERVER â€” COMPLETE & FINAL VERSION');
  console.log(`Server running on http://localhost:${PORT}`);
  console.log(`Admin panel: http://localhost:${PORT}/admin-limits`);
  console.log('Features:');
  console.log('  â€¢ Strict per-form rate limiting (10 per IP per form / 15 min)');
  console.log('  â€¢ No automatic redirect after successful payment');
