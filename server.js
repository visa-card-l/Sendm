...
// === ALL YOUR ORIGINAL AUTH ROUTES (UNCHANGED) ===
app.post('/api/auth/register', authLimiter, async (req, res) => { /* ... same ... */ });
app.post('/api/auth/login', authLimiter, async (req, res) => { /* ... same ... */ });
app.get('/api/auth/me', authenticateToken, (req, res) => { /* ... same ... */ });
app.post('/api/auth/connect-telegram', authenticateToken, async (req, res) => { /* ... same ... */ });
...
app.get('/api/pages', authenticateToken, (req, res) => { /* ... same ... */ });
...
app.post('/api/pages/delete', authenticateToken, (req, res) => { /* ... same ... */ });

// === FULL SSR RENDERING ROUTE (THIS IS THE ONLY CHANGE) ===
app.get('/p/:shortId', (req, res) => {
  ...
  res.render('landing', {
    title: page.title,
    blocks: page.config.blocks  // ← raw array, no JSON.stringify!
  });
});

// === CREATE VIEWS FOLDER + SSR TEMPLATES ===
...
// FULL SSR landing.ejs — 100% server-rendered
...
<head>
  ...
  <style>
    ...
    *{margin:0;padding:0;box-sizing:border-box;}
    ...
    p{font-size:17px;color:var(--gray-600);margin-bottom:32px;line-height:1.7;}
...
<body>
  <div class="wrapper">
    <% blocks.forEach(block => { %>
      <% if (block.type === 'text') { %>
        <<%= block.tag || 'p' %> style="margin-bottom: 20px; font-size: <%= block.tag === 'h1' ? '42px' : block.tag === 'h2' ? '34px' : '17px' %>;">
          <%= block.content %>
        </<%= block.tag || 'p' %>>
      <% } else if (block.type === 'image') { %>
        <div style="text-align:center;margin:40px 0;">
          <img src="<%= block.src %>" alt="<%= block.alt || '' %>" class="landing-image">
        </div>
      <% } else if (block.type === 'button') { %>
        <a href="<%= block.href || '#' %>" 
           target="_blank" 
           class="cta-button"
           style="background:<%= block.bgColor || 'var(--primary)' %>;color:<%= block.textColor || 'white' %>;">
...
app.listen(PORT, () => {
  ...
  console.log('Editor → add ?token=your_jwt');
  console.log('Pages → /p/shortid');
});
