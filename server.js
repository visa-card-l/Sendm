...
const fetch = require('node-fetch'); // Required for bot validation
...
const ACTIVATION_NONCES = new Map(); // nonce → userId
...
function launchUserBot(user, activationNonce = null) {
  ...
  bot.start(async (ctx) => {
    ...
    // Allow normal /start if already connected
...
// Register — EXACTLY LIKE PLEXZORA
app.post('/api/auth/register', authLimiter, async (req, res) => {
  try {
    ...
    const newUser = {
      id: Date.now().toString(),           // EXACT SAME AS PLEXZORA
      fullName,
...
// Connect Telegram — FIXED DEEP LINK
app.post('/api/auth/connect-telegram', authenticateToken, async (req, res) => {
  ...
  try {
    ...
    // CORRECT DEEP LINK (this was the bug!)
    ...
    res.json({
      ...
      startLink,
      instructions: 'Open link → Tap START → Done!'
    });
  } catch (err) {
